    ...
    >
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="消息" prop="xiaoxi">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.xiaoxi" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.xiaoxi" label="消息" prop="xiaoxi">
                    <span v-html="ruleForm.xiaoxi"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
	...
	this.ruleForm.xiaoxi = this.ruleForm.xiaoxi.replace(reg,'../../../springboothch86/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `qiyexiaoxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.qiyexiaoxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
