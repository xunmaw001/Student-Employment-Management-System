<template>
  <div class="main-content">
        ...
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
            ...
            <el-table-column width="300" :align="contents.tableAlign" 
               :header-align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('xuesheng','学生档案') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="xueshengdanganCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'学生档案':'' }}</el-button>
                <el-button v-if="isAuth('xuesheng','学生档案') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="xueshengdanganCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'学生档案':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('xuesheng','学生档案') && contents.tableBtnIcon == 0" type="success" size="mini" @click="xueshengdanganCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'学生档案':'' }}</el-button>
    ...
    <xueshengdangan-cross-add-or-update v-if="xueshengdanganCrossAddOrUpdateFlag" :parent="this" ref="xueshengdanganCrossaddOrUpdate"></xueshengdangan-cross-add-or-update>
...
import xueshengdanganCrossAddOrUpdate from "../xueshengdangan/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      xueshengdanganCrossAddOrUpdateFlag: false,
  ...
  components: {
    AddOrUpdate,
    xueshengdanganCrossAddOrUpdate,
  },
  methods: {
    ...
    xueshengdanganCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.xueshengdanganCrossAddOrUpdateFlag = true;
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.xueshengdanganCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.xueshengdanganCrossaddOrUpdate.init(row.id,type);
      });
    },
    init () {
    },
