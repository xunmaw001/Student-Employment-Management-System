<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="工作类型" prop="gongzuoleixing">
          <el-input v-model="ruleForm.gongzuoleixing" 
              placeholder="工作类型" clearable  :readonly="ro.gongzuoleixing"></el-input>
        </el-form-item>
        <div v-else>
      ...
      <el-col :span="24">  
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.jianliwenjian" label="简历文件" prop="jianliwenjian">
            <el-button type="text" size="small" @click="download($base.url+ruleForm.jianliwenjian)">下载</el-button>
          </el-form-item>
        </div>    
      </el-col>      
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="投简时间" prop="toujianshijian">
            <el-date-picker
                value-format="yyyy-MM-dd HH:mm:ss"
                v-model="ruleForm.toujianshijian" 
                type="datetime"
                :readonly="ro.toujianshijian"
                placeholder="投简时间">
            </el-date-picker>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.toujianshijian" label="投简时间" prop="toujianshijian">
              <el-input v-model="ruleForm.toujianshijian" 
                placeholder="投简时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  ...
  created() {
	this.ruleForm.toujianshijian = this.getCurDateTime()
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.jianliwenjian = true;
          ...
          if(o=='toujianshijian'){
            this.ruleForm.toujianshijian = obj[o];
	    this.ro.toujianshijian = true;
            continue;
          }
        }
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `toujianxinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.toujianxinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
